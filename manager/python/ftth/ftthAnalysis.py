    ...
    def execute_query(self, query: str, params=None) -> List[Dict]:
        ...
        try:
            ...
            if cursor.description:
                ...
                print("DEBUG: Query executed successfully (no return data)")
                return []
        except Exception as e:
    ...
    def get_shortest_path_analysis(
        ...
        try:
            ...
            if virtual_shaft_info.get("start_virtual_shaft_id"):
                ...
                print(f"DEBUG: Looking for start virtual shaft ID: {start_virtual_id}")
                start_shaft = None
                for shaft in shafts_data:
                ...
                if start_shaft:
                    ...
                    print(f"DEBUG: Found start virtual shaft: {start_shaft['id']} at ({start_shaft['x']}, {start_shaft['y']})")
                else:
                    # Fallback to nearest shaft if virtual shaft not found
                    print(f"DEBUG: Virtual shaft {start_virtual_id} not found, falling back to nearest")
                    start_shaft, start_offset_distance = find_nearest_shaft(start_point)
            else:
                start_shaft, start_offset_distance = find_nearest_shaft(start_point)
                print(f"DEBUG: Found nearest start shaft: {start_shaft['id']} at ({start_shaft['x']}, {start_shaft['y']}) distance: {start_offset_distance}")
            ...
            if virtual_shaft_info.get("end_virtual_shaft_id"):
                ...
                print(f"DEBUG: Looking for end virtual shaft ID: {end_virtual_id}")
                end_shaft = None
                for shaft in shafts_data:
                ...
                if end_shaft:
                    ...
                    print(f"DEBUG: Found end virtual shaft: {end_shaft['id']} at ({end_shaft['x']}, {end_shaft['y']})")
                else:
                    # Fallback to nearest shaft if virtual shaft not found
                    print(f"DEBUG: Virtual shaft {end_virtual_id} not found, falling back to nearest")
                    end_shaft, end_offset_distance = find_nearest_shaft(end_point)
            else:
                end_shaft, end_offset_distance = find_nearest_shaft(end_point)
                print(f"DEBUG: Found nearest end shaft: {end_shaft['id']} at ({end_shaft['x']}, {end_shaft['y']}) distance: {end_offset_distance}")
            ...
            print(f"DEBUG: Executing pgr_bellmanFord query with start_shaft_id={start_shaft_id}, end_shaft_id={end_shaft_id}")
            print(f"DEBUG: Query: {path_query}")
            
            path_result = self.execute_query(path_query, (start_shaft_id, end_shaft_id))
            
            print(f"DEBUG: Path result: {path_result}")
            
            if not path_result:
                # Check if pgRouting extension is available
                check_pgrouting = self.execute_query("SELECT proname FROM pg_proc WHERE proname = 'pgr_bellmanford'")
                pgrouting_available = len(check_pgrouting) > 0
                
                # Check if the edges table has the required columns
                check_edges = self.execute_query(f"""
                    SELECT column_name 
                    FROM information_schema.columns 
                    WHERE table_name = '{self.tables['edges'].split('.')[-1]}' 
                    AND column_name IN ('cost', 'reverse_cost', 'shaft_1', 'shaft_2')
                """)
                required_columns = [row['column_name'] for row in check_edges]
