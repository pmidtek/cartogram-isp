            ...
            with conn.cursor() as cur:
                ...
                if total_input_table == 2:
                    select_query = sql.SQL(
                            ...
                            FROM (
                              ...
                              FROM 
                                {table_a}, ( SELECT
                                                ST_Union (geom) geom
                                            FROM {table_b} 
                                            {where_clause_table_b}
                                            ) AS u
                                {where_clause_table_a}
                            ) differenced
                        ...
                        where_clause_table_a=sql.SQL(
                            build_where_clause([input_table[0]])
                        ),
                        where_clause_table_b=sql.SQL(
                            build_where_clause([input_table[1]])
                        ),
                    )
                else:
                ...
                cur.execute(
                    sql.SQL(
                        f"""INSERT INTO vector_tiles_directus_roles 
