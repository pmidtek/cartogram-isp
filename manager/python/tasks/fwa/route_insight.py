...
def route_insight(
    ...
    try:
        ...
        geojson_tower_input = result.get("geojson_tower", None)

        options = result.get("options", None)
        area_city_ids = options.get("area_city_ids", [])

        features = geojson_tower_input["features"]
        points = []
        for ft in features:
            geom = shape(ft["geometry"])
            points.append(
                {
                    "id": str(ft["properties"].get("id")),
                    "type": ft["properties"].get("type", "Tower"),
        ...
        with conn.cursor() as cur:
            cur.execute(
                """
                    SELECT sp.*
                    FROM site_points sp
                    INNER JOIN site_point_types spt ON sp.site_point_type_id = spt.id AND spt.name IN ('Stasiun', 'Shelter')
                    WHERE ST_Within(sp.geom,ST_MakeEnvelope(%s, %s, %s, %s, 4326))
                    ORDER BY sp.id;
                """,
                (xmin, ymin, xmax, ymax),
            )
            ...
            if rows:
                ...
                logger.info(
                    "No station found inside bbox, searching nearest point instead..."
                )
                cur.execute(
                    """
                        SELECT sp.*
                        FROM site_points sp
                        INNER JOIN site_point_types spt ON sp.site_point_type_id = spt.id AND spt.name IN ('Stasiun', 'Shelter')
                        ORDER BY sp.geom <-> ST_SetSRID(ST_MakePoint(%s, %s), 4326)
                        LIMIT 1;
                    """,
                    (centroid_x, centroid_y),
                )
                stasiun_list = cur.fetchall()

            for row in stasiun_list:
                cols = [desc[0] for desc in cur.description]
                st = dict(zip(cols, row))
                geom = wkb.loads(bytes.fromhex(st["geom"]))
        ...
        with conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO route_insight_result(date_created, name, user_created, result, geoprocessing_uuid, excel)
