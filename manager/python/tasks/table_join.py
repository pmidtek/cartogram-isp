...
@dramatiq.actor(store_results=True, time_limit=3000000)
def table_join(
    target_table: str,
    join_table: str,
    target_id: str,
    ...
    try:
        ...
        with conn:
            with conn.cursor() as cur:
                logger.info(
                    f"Starting table_join with: {target_table}, {join_table}, {target_id}, {join_id}"
                )
                ...
                # Check that join_table exists (optional but useful for debugging)
                cur.execute(
                ...
                if not table_columns:
                    raise Exception("No columns found in input tables")
                ...
                cur.execute(
                    sql.SQL(
                        """ INSERT INTO {output_table} ({output_fields},geom)
                            ...
                            FROM {target_table}
                            LEFT JOIN {join_table} ON {target_table}.{target_id} = {join_table}.{join_id}
                            {filter} """
                    ).format(
                        output_table=output_table_ident,
                        ...
                        target_id=sql.Identifier(target_id),
                        join_id=sql.Identifier(join_id),
                        filter=(
