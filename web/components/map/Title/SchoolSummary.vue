...
const { toggleTowerLayersByOwner, toggleSchoolLayersByJenjang } = mapLayerStore;
...
const schoolOwnerOptions = [
  { value: "SD", label: "SD" },
  { value: "PAUD", label: "PAUD" },
  { value: "SMP", label: "SMP" },
  { value: "SMA", label: "SMA" },
  { value: "SMK", label: "SMK" },
  { value: "PNFI", label: "PNFI" },
  { value: "SLB", label: "SLB" },
];
...
const { data: towerData, isLoading: isLoadingTowers } = useQuery({
  queryKey: [
    "/panel/chart/count/poi-school-jenjang",
    selectedProvince,
    selectedCity,
  ],
  queryFn: async () => {
    let url = "/panel/chart/count/poi-school-jenjang?";
...
const { data: top10Data, isLoading: isLoadingTop10 } = useQuery({
  queryKey: ["/panel/chart/count/poi-school-area", selectedTowerOwner],
  queryFn: async () => {
    if (!selectedTowerOwner.value) return null;
    return await $fetch<Top10Response>(
      `/panel/chart/count/poi-school-area?owner=${selectedTowerOwner.value}&group_by=province&limit=10`,
      {
        headers: { Authorization: `Bearer ${authStore.accessToken}` },
      },
    );
...
const { data: cityDetailData, isLoading: isLoadingCityDetail } = useQuery({
  queryKey: [
    "/panel/chart/count/poi-school-area",
    selectedTowerOwner,
    isTop10ModalOpen,
  ],
  queryFn: async () => {
    ...
    return await $fetch<CityDetailResponse>(
      `/panel/chart/count/poi-school-area?owner=${selectedTowerOwner.value}&group_by=city&limit=1000`,
      {
        headers: { Authorization: `Bearer ${authStore.accessToken}` },
      },
    );
  ...
  datasets: [
    {
      ...
      backgroundColor:
        towerData.value?.data.list.map((item) => {
          const baseColor = schoolColors[item.owner] || fallbackColors[0];
    ...
    tooltip: {
      ...
      callbacks: {
        label: (context: any) => {
          ...
          return `School: ${value.toLocaleString()}`;
        },
      },
    },
  },
  ...
  datasets: [
    {
      ...
      backgroundColor: selectedTowerOwner.value
        ? schoolColors[selectedTowerOwner.value] || "#6B7280"
        : "#6B7280",
      borderRadius: 4,
      barThickness: 20,
    },
    ...
    tooltip: {
      ...
      callbacks: {
        label: (context: any) => {
          ...
          return `${context.parsed.x.toLocaleString()} school (${item?.percentage}%)`;
        },
      },
    },
  },
...
const handleClearFilters = () => {
  ...
  toggleSchoolLayersByJenjang(null, null, null);
};
...
watch([selectedProvince, selectedCity], () => {
  // Reset jenjang filter when area changes
  if (selectedTowerOwnerForMap.value) {
    selectedTowerOwnerForMap.value = null;
  }

  // Apply area filter (combines with jenjang if selected)
  toggleSchoolLayersByJenjang(
    selectedTowerOwnerForMap.value,
    selectedProvince.value,
    selectedCity.value,
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            <div
              ...
              :style="{
                backgroundColor:
                  schoolColors[selectedTowerOwnerForMap] || '#6B7280',
              }"
            ></div>
            <p class="text-sm text-grey-900 font-medium">
              Showing: {{ selectedTowerOwnerForMap }}
            </p>
          </div>
          <button
            @click="
              () => {
                ...
                toggleSchoolLayersByJenjang(null);
              }
            "
    ...
    <div class="flex-1 overflow-y-auto px-3 pb-3 max-h-[calc(100vh-26rem)]">
      <div class="space-y-4">
        ...
        <div class="bg-grey-50 rounded-xs p-3 border border-grey-200">
          <p class="text-grey-600 text-xs">Total Schools</p>
        ...
        <div class="space-y-3">
          ...
          <USelect
            ...
            :options="schoolOwnerOptions"
            placeholder="Select Jenjang School"
            size="sm"
            :ui="{ rounded: 'rounded-xxs' }"
          />
            ...
            <div class="flex items-center justify-between mb-3">
              ...
              <div class="flex items-center gap-3">
                <span class="text-xs text-grey-600">
                  Total: {{ formatNumber(top10Data.data.total) }} schools
                </span>
                <UButton
                  size="xs"
                  variant="outline"
            ...
            >
              <div class="flex items-center gap-2">
                <div
                  ...
                  :style="{
                    backgroundColor:
                      schoolColors[item.owner] || fallbackColors[0],
                  }"
                ></div>
                <p>{{ item.owner }}</p>
              </div>
      ...
      <template #header>
        <div class="flex items-center justify-between">
          <h3 class="text-lg font-semibold text-grey-900">
            City Distribution - {{ selectedTowerOwner }} School
          </h3>
          <UButton
            color="gray"
            variant="ghost"
      ...
      <div v-else-if="cityDetailData?.data.list" class="space-y-4">
        ...
        <div class="grid grid-cols-3 gap-4">
          <div class="bg-grey-50 rounded-lg p-4 border border-grey-200">
            <p class="text-xs text-grey-600 mb-1">Total School</p>
          ...
          <div class="bg-grey-50 rounded-lg p-4 border border-grey-200">
            ...
            <div class="flex items-center gap-2">
              <div
                ...
                :style="{
                  backgroundColor:
                    schoolColors[selectedTowerOwner || ''] || '#6B7280',
                }"
              ></div>
              <p class="text-2xl font-bold text-grey-900">
                {{ selectedTowerOwner }}
